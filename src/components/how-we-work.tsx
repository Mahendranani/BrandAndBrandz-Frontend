...
function VideoItem({ src, align }: VideoItemProps) {
    ...
    return (
        <div ref={containerRef} className={`w-full md:w-1/2 flex mb-8 md:mb-0 ${align === 'right' ? 'md:justify-end md:pr-12' : 'md:justify-start md:pl-12'} justify-center`}>
            <motion.div
...
function StepInfo({ title, desc, align }: { title: string; desc: string; align: "left" | "right" }) {
    return (
        <div className={`w-full md:w-1/2 mb-8 md:mb-0 ${align === 'right' ? 'md:text-right md:pr-12 text-center' : 'md:text-left md:pl-12 text-center'}`}>
            <h3 className="text-2xl md:text-3xl font-bold text-white mb-2">{title}</h3>
            <p className="text-white/70 text-lg md:text-lg">{desc}</p>
        </div>
    );
}
...
export function HowWeWork() {
    ...
    return (
        <section ref={containerRef} className="py-24 px-4 sm:px-6 lg:px-8 bg-black relative overflow-hidden">
            <h2 className="text-center text-3xl sm:text-4xl font-bold text-white mb-4">How We Work</h2>
            ...
            <div className="container mx-auto max-w-5xl relative">
                ...
                {/* DESKTOP Timeline Lines (Hidden on Mobile) */}
                <div className="hidden md:block absolute left-1/2 -translate-x-1/2 w-[2px] h-full bg-white/5 top-0 z-0 rounded-full" />
                <motion.div
                    style={{ height: lineHeight }}
                    className="hidden md:block absolute left-1/2 -translate-x-1/2 w-[2px] bg-gradient-to-b from-zinc-400 via-[#70879f] to-sky-700 top-0 z-10 origin-top rounded-full shadow-[0_0_15px_rgba(59,130,246,0.6)] backdrop-blur-sm"
                />
                            ...
                            >
                                ...
                                */}
                                ...
                                <div className={`w-full md:w-1/2 flex justify-center ${isEven ? 'md:justify-end' : 'md:justify-start'} order-2 md:order-1`}>
                                    ...
                                    {/* Mobile Only: For Even steps, we hid the Video above, so we need to show Text here if we want Text First on Mobile? 
                                         Actually, let's keep it simple. Stack naturally.
                                         The easiest mobile layout is Image -> Text.
                                     */}
                                    {isEven && (
                                        <div className="md:hidden w-full"><VideoItem src={step.video} align="left" /></div>
                                    )}
                                </div>
                                ...
                                <div className={`w-full md:w-1/2 flex justify-center ${isEven ? 'md:justify-start' : 'md:justify-end'} order-3`}>
                                    ...
                                    {/* Mobile Only: For Odd steps, we hid the Video above, so show it here */}
                                    {!isEven && (
                                        <div className="md:hidden w-full"><VideoItem src={step.video} align="left" /></div>
                                    )}
                                </div>
